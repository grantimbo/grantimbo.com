...
export default async function FeaturedProjects({
  ...
  return (
    <section
      className={`faded bg-drkblue relative flex flex-col items-center px-8 py-4 md:px-20 ${!hideHeader ? "md:py-40" : "md:py-5"}`}
    >
      {!hideHeader && (
      ...
      <section className="relative z-10 mb-10 grid max-w-7xl grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {projectCats?.map((x) => (
          <article
            key={x?.title}
